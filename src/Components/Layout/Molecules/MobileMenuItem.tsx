...
export const MobileMenuItem = ({ path, title, blank, children, isSub }: Props) => {
                ...
                <ul className='flex flex-col'>
                  {
                    children.map((item, idx) => (
                      <MobileMenuItem isSub key={idx + 1} {...item} path={`${path}/${item.path}`} />
                    ))
                  }
                </ul>
              }
            ...
            <>
              {blank ? (
                ...
                <Link href={path} locale={locale}>
                  <a className='p-3 font-semibold border-b-[1px] border-[#6A6D82] text-sm block'>
                    {t(title)}
                  </a>
                </Link>
              )}
